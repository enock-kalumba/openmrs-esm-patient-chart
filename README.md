...
You could run `yarn start` with as many `sources` arguments as you require. For example, to run the vitals microfrontend, you'd use:

```bash
yarn start --sources 'packages/esm-patient-vitals-app'
```

To run multiple microfrontends simultaneously, such as vitals and conditions, you'd use:

```bash
yarn start --sources 'packages/esm-patient-vitals-app' --sources 'packages/esm-patient-conditions-app'
```
